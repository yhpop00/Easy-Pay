...
while($res = $rs->fetch())
{
echo '<tr><td><b>'.$res['id'].'</b></td><td><b>'.($res['uid']>0?'<a href="./ulist.php?column=uid&value='.$res['uid'].'" target="_blank">'.$res['uid'].'</a>':'管理员').'</b></td><td>'.$res['type'].'</td><td><a href="https://m.ip138.com/iplookup.asp?ip='.$res['ip'].'" target="_blank" rel="noreferrer">'.$res['ip'].'</a></td><td>'.$res['date'].'</td></tr>';
}
?>
          </tbody>
        </table>
