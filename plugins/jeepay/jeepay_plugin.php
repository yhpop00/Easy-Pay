...
class jeepay_plugin
{
	...
	static public function submit(){
		...
		if($order['typename']=='alipay'){
			...
			if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')!==false && in_array('7',$channel['apptype']) && $channel['appwxmp']>0){
	...
	static public function mapi(){
		...
		if($order['typename']=='alipay'){
			...
			if($mdevice=='wechat' && in_array('7',$channel['apptype']) && $channel['appwxmp']>0){
	...
	static public function wxpay(){
		try{
			list($type, $payData) = self::addOrder('WX_NATIVE');
		}catch(Exception $ex){
	...
	static public function notify(){
		...
		if($sign===$_POST["sign"]){
			if($_POST['state'] == '2'){
				...
				return ['type'=>'html','data'=>'state='.$_POST['state']];
			}
		}else{
